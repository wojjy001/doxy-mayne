...
shinyServer(function(input,output,session) {
	...
	# Plot simulation results of fed versus fasted for Doryx MPC
		output$RdoryxMPC.plot2 <- renderPlot({
			...
			# Fed
				plotobj1 <- plotobj1 + geom_line(aes(x = time,y = Median),data = doryxMPC.summary2[doryxMPC.summary2$FED == 1,],colour = "blue")
				if (input$PI > 1) plotobj1 <- plotobj1 + geom_ribbon(aes(x = time,ymin = CIlo,ymax = CIhi),data = doryxMPC.summary2[doryxMPC.summary2$FED == 1,],fill = "blue",alpha = 0.3)
	...
	# Plot simulation results of fed versus fasted for Doryx Tablet
		output$RdoryxTAB.plot2 <- renderPlot({
			...
			# Fasted
				plotobj2 <- plotobj2 + geom_line(aes(x = time,y = Median),data = doryxTAB.summary2[doryxTAB.summary2$FED == 0,],colour = "red")
				if (input$PI > 1) plotobj2 <- plotobj2 + geom_ribbon(aes(x = time,ymin = CIlo,ymax = CIhi),data = doryxTAB.summary2[doryxTAB.summary2$FED == 0,],fill = "red",alpha = 0.3)
			# Fed
	...
	# Summary table of fed versus fasted for Doryx MPC
		output$RdoryxMPC.table2 <- renderTable({
		  # Read in the necessary reactive expressions
			  doryxMPC.data2 <- RdoryxMPC.data2()
			  summary.function <- Rsummary.function()
			  if (input$DOSE1 == 1) {
					# Summarise at t = 96 hours for single dose scenarios
						doryxMPC.data96 <- subset(doryxMPC.data2,time == 96)
						# Summarise AUC
					...
					# Return data frame
						doryxMPC.table2 <- rbind(AUC.table,Cmax.table,Tmax.table)
						doryxMPC.table2$FED[doryxMPC.table2$FED== 0] <- "Fasted"
						doryxMPC.table2$FED[doryxMPC.table2$FED== 1] <- "Fed"
						if (input$PI == 1) {doryxMPC.table2 <- data.frame(Status = doryxMPC.table2$FED,Median = doryxMPC.table2$Median,Variable = doryxMPC.table2$Variable)
						}
						if (input$PI > 1) {doryxMPC.table2 <- data.frame(Status = doryxMPC.table2$FED,Median = doryxMPC.table2$Median,CIlo = doryxMPC.table2$CIlo,CIhi = doryxMPC.table2$CIhi,Variable = doryxMPC.table2$Variable)
						}
			  }
			  
			  if (input$DOSE1 != 1) {
					# Summarise at t = 240 for multiple dose scenario
				  	doryxMPC.data240 <- subset(doryxMPC.data2,time == 240)
						# Summarise AUC
	...
	# Summary table of fed versus fasted for Doryx TAB
		output$RdoryxTAB.table2 <- renderTable({
		  # Read in the necessary reactive expressions
		  doryxTAB.data2 <- RdoryxTAB.data2()
		  summary.function <- Rsummary.function()
		  if (input$DOSE1 == 1) {
		    # Summarise at t = 96 hours for single dose scenarios
		    doryxTAB.data96 <- subset(doryxTAB.data2,time == 96)
		    # Summarise AUC
		    ...
		    Tmax.table$Variable <- "Tmax (h)"
		    # Return data frame
		    doryxTAB.table2 <- rbind(AUC.table,Cmax.table,Tmax.table)
		    doryxTAB.table2$FED[doryxTAB.table2$FED== 0] <- "Fasted"
		    doryxTAB.table2$FED[doryxTAB.table2$FED== 1] <- "Fed"
		    if (input$PI == 1) {doryxTAB.table2 <- data.frame(Status = doryxTAB.table2$FED,Median = doryxTAB.table2$Median,Variable = doryxTAB.table2$Variable)
		    }
		    if (input$PI > 1) {doryxTAB.table2 <- data.frame(Status = doryxTAB.table2$FED,Median = doryxTAB.table2$Median,CIlo = doryxTAB.table2$CIlo,CIhi = doryxTAB.table2$CIhi,Variable = doryxTAB.table2$Variable)
		    }
		  }
		  
		  if (input$DOSE1 != 1) {
		    # Summarise at t = 240 for multiple dose scenario
		    doryxTAB.data240 <- subset(doryxTAB.data2,time == 240)
		    # Summarise AUC
